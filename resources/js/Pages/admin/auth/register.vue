...
const login = async () => {
    ...
    axios.post(route('register'), form).then((res) => {
...
<template>
    ...
    <div class="min-h-screen h-full flex flex-col after:flex-1">
        ...
        <div class="max-w-sm p-8 w-full border rounded-md shadow-md">
            ...
            <form>
                <div class="space-y-4">
                    <VInput type="text" placeholder="Your name" label="Name" v-model="form.name"/>
                    ...
                    <VInput :type="show ? 'text' : 'password'" placeholder="Password Confirmation" label="Password Confirmation" v-model="form.password_confirmation">
                        <template v-slot:icon>
                            <span class="absolute inset-y-0 right-0 flex items-center pr-2 cursor-pointer" @click="show = !show">
                                <svg v-if="!show" width="24" height="24" viewBox="0 0 12 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M2.5 7L10 15.25" stroke="#475569" stroke-linecap="round" stroke-linejoin="round" />
                ...
                <div class="flex items-center justify-end mt-6">
                    <VButton :is-loading="isLoading" @click="login" label="Register" />
                </div>
            </form>
            <div class="pt-5 mt-6 border-t border-slate-200">
                <div class="text-sm">
                    <Link class="font-medium text-primary hover:text-primary-hover underline" :href="route('login')">
